...
4. **Phase 4 — Chapter revision.** Each chapter: packet → Claude revision (with mid-draft flagging) → defensive audits (Claude-tic, calibration Gates, inviolables, register preservation, adversarial register) → synthesis → integration → mechanical quality gate → core audit stack (consequence, opening hook, ending quality, surprise, threads, continuity, voice, scored editor check, convenient-invention fact-check, counter-strip, dialogue doctor) → trigger-fired risk audits (plain-translation, comedy doctor, clarity, set-piece, calibration-gated amplification audits, ...) → tracker updates → commit.
...
6. **Phase 6 — Final assembly.** Audit-coverage verification (every chapter x every applicable audit), Final Test + required checks, LLM-judge rubric, mandatory full-book Claude cold read, locked-line diff, final mechanical checks. Assemble final manuscript.
