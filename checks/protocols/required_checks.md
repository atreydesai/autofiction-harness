...
purpose: the writing ruleset's mandatory self-audit battery, plus the optional long-form diagnostics used when the required checks are not enough
runs: on every substantial revision pass (drafter self-edit and editor-stage passes); the short-piece subset rule governs scene-sized fragments, the full battery governs chapters and larger units
source_ref: collation:3037-3052 [Required checks]; collation:3060-3075 [Optional long-form diagnostics]
...
## Optional long-form diagnostics, verbatim
...
- Fiction mapping: "the medium and the user's request" reads as the premise, reading guide, and calibration artifact; "the writer's view" in check 7 reads as narrative stance and viewpoint consciousness; checks 3-4 apply to any real-world facts, quotes, or causal claims the manuscript leans on.
- Check 10 is binding in both directions: the over-correction it bans is also banned by the Safety Rails in checks/protocols/precedence.md.
- "Do not output the audit unless asked" — in the harness, the audit IS asked for: revision-pass reports record which checks fired and what was rewritten, but no audit text may leak into the manuscript itself.
